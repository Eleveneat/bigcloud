"""empty message

Revision ID: 758fbba26403
Revises: 
Create Date: 2017-03-20 11:17:45.083772

"""
from alembic import op
import sqlalchemy as sa
import sqlalchemy_utils


# revision identifiers, used by Alembic.
revision = '758fbba26403'
down_revision = None
branch_labels = None
depends_on = None


def upgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.create_table('appgroups',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('应用组名', sa.Unicode(length=255), nullable=True),
    sa.Column('描述', sa.Unicode(length=255), nullable=True),
    sa.Column('创建时间', sa.DateTime(), nullable=True),
    sa.Column('修改时间', sa.DateTime(), nullable=True),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('应用组名'),
    sa.UniqueConstraint('描述')
    )
    op.create_table('cpu',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('CPU', sa.Unicode(length=255), nullable=True),
    sa.Column('创建时间', sa.DateTime(), nullable=True),
    sa.Column('修改时间', sa.DateTime(), nullable=True),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('CPU')
    )
    op.create_table('filegroups',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('文件组名', sa.Unicode(length=255), nullable=True),
    sa.Column('描述', sa.Unicode(length=255), nullable=True),
    sa.Column('创建时间', sa.DateTime(), nullable=True),
    sa.Column('修改时间', sa.DateTime(), nullable=True),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('描述'),
    sa.UniqueConstraint('文件组名')
    )
    op.create_table('files',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('文件名', sa.Unicode(length=255), nullable=True),
    sa.Column('文件总大小', sa.Integer(), nullable=True),
    sa.Column('MD5', sa.String(length=255), nullable=True),
    sa.Column('创建时间', sa.DateTime(), nullable=True),
    sa.Column('修改时间', sa.DateTime(), nullable=True),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('文件名')
    )
    op.create_table('first_classifications',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('一级分类名', sa.Unicode(length=255), nullable=True),
    sa.Column('创建时间', sa.DateTime(), nullable=True),
    sa.Column('修改时间', sa.DateTime(), nullable=True),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_table('languages',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('语言', sa.Unicode(length=255), nullable=True),
    sa.Column('创建时间', sa.DateTime(), nullable=True),
    sa.Column('修改时间', sa.DateTime(), nullable=True),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('语言')
    )
    op.create_table('littleclouds',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('小云平台名称', sa.Unicode(length=255), nullable=True),
    sa.Column('小云平台URL', sa.String(length=255), nullable=True),
    sa.Column('是否允许接入', sa.Boolean(), nullable=True),
    sa.Column('是否已经接入', sa.Boolean(), nullable=True),
    sa.Column('联系电话', sa.String(length=30), nullable=True),
    sa.Column('联系邮箱', sqlalchemy_utils.types.email.EmailType(length=255), nullable=True),
    sa.Column('接入IP', sqlalchemy_utils.types.ip_address.IPAddressType(length=50), nullable=True),
    sa.Column('接入端口', sa.Integer(), nullable=True),
    sa.Column('接入协议', sa.String(length=20), nullable=True),
    sa.Column('创建时间', sa.DateTime(), nullable=True),
    sa.Column('修改时间', sa.DateTime(), nullable=True),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('小云平台URL'),
    sa.UniqueConstraint('小云平台名称')
    )
    op.create_table('os',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('操作系统', sa.Unicode(length=255), nullable=True),
    sa.Column('创建时间', sa.DateTime(), nullable=True),
    sa.Column('修改时间', sa.DateTime(), nullable=True),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('操作系统')
    )
    op.create_table('packages',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('文件名', sa.Unicode(length=255), nullable=True),
    sa.Column('文件总大小', sa.Integer(), nullable=True),
    sa.Column('MD5', sa.String(length=255), nullable=True),
    sa.Column('创建时间', sa.DateTime(), nullable=True),
    sa.Column('修改时间', sa.DateTime(), nullable=True),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('文件名')
    )
    op.create_table('users',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('用户名', sa.String(length=50), nullable=True),
    sa.Column('邮箱', sa.String(length=120), nullable=True),
    sa.Column('哈希加密密码', sa.String(length=128), nullable=True),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('用户名')
    )
    op.create_table('filegroup_file',
    sa.Column('filegroup_id', sa.Integer(), nullable=True),
    sa.Column('file_id', sa.Integer(), nullable=True),
    sa.ForeignKeyConstraint(['file_id'], ['files.id'], ),
    sa.ForeignKeyConstraint(['filegroup_id'], ['filegroups.id'], )
    )
    op.create_table('littlecloud_appgroup',
    sa.Column('littlecloud_id', sa.Integer(), nullable=True),
    sa.Column('appgroup_id', sa.Integer(), nullable=True),
    sa.ForeignKeyConstraint(['appgroup_id'], ['appgroups.id'], ),
    sa.ForeignKeyConstraint(['littlecloud_id'], ['littleclouds.id'], )
    )
    op.create_table('secondary_classifications',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('二级分类名', sa.Unicode(length=255), nullable=True),
    sa.Column('first_classification_id', sa.Integer(), nullable=True),
    sa.Column('创建时间', sa.DateTime(), nullable=True),
    sa.Column('修改时间', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['first_classification_id'], ['first_classifications.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_table('functions',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('功能名', sa.Unicode(length=255), nullable=True),
    sa.Column('secondary_classification_id', sa.Integer(), nullable=True),
    sa.Column('创建时间', sa.DateTime(), nullable=True),
    sa.Column('修改时间', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['secondary_classification_id'], ['secondary_classifications.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('功能名')
    )
    op.create_table('applications',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('应用名', sa.Unicode(length=255), nullable=True),
    sa.Column('版本号', sa.String(length=255), nullable=True),
    sa.Column('function_id', sa.Integer(), nullable=True),
    sa.Column('language_id', sa.Integer(), nullable=True),
    sa.Column('cpu_id', sa.Integer(), nullable=True),
    sa.Column('os_id', sa.Integer(), nullable=True),
    sa.Column('package_id', sa.Integer(), nullable=True),
    sa.Column('安装命令', sa.String(length=255), nullable=True),
    sa.Column('创建时间', sa.DateTime(), nullable=True),
    sa.Column('修改时间', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['cpu_id'], ['cpu.id'], ),
    sa.ForeignKeyConstraint(['function_id'], ['functions.id'], ),
    sa.ForeignKeyConstraint(['language_id'], ['languages.id'], ),
    sa.ForeignKeyConstraint(['os_id'], ['os.id'], ),
    sa.ForeignKeyConstraint(['package_id'], ['packages.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('应用名')
    )
    op.create_table('appgroup_application',
    sa.Column('appgroup_id', sa.Integer(), nullable=True),
    sa.Column('application_id', sa.Integer(), nullable=True),
    sa.ForeignKeyConstraint(['appgroup_id'], ['appgroups.id'], ),
    sa.ForeignKeyConstraint(['application_id'], ['applications.id'], )
    )
    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_table('appgroup_application')
    op.drop_table('applications')
    op.drop_table('functions')
    op.drop_table('secondary_classifications')
    op.drop_table('littlecloud_appgroup')
    op.drop_table('filegroup_file')
    op.drop_table('users')
    op.drop_table('packages')
    op.drop_table('os')
    op.drop_table('littleclouds')
    op.drop_table('languages')
    op.drop_table('first_classifications')
    op.drop_table('files')
    op.drop_table('filegroups')
    op.drop_table('cpu')
    op.drop_table('appgroups')
    # ### end Alembic commands ###
